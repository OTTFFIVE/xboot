...
## examples
这里主要存放各种参考测试例程，按主题内容，分目录存放。在编写应用程序时，应首先参考此目录里的相关实现，加快开发进度，降低学习成本，源码是最好的老师。
...
#### arm32
该目录下实现了很多以`mach-`开头的目录，这些都是同一体系下的不同板级支持包，aarch32指令集，包括`arm7` `arm9` `cortex-a8` `cortex-a9` `cortex-a15` `cortex-a17` `cortex-a5` `cortex-a7`等，已实现的板子支持包列举如下：
...
#### arm64
该目录主要实现aarch64指令集，包括`cortex-a53` `cortex-a72`等，目录组织方式同`arm32`，已板子支持包列举如下：
...
#### x64
该目录，就是传统的PC平台，`X86_64`指令集，目录组织方式同`arm32`，这里仅实现了沙盒，运行在PC操作系统之上。
...
### framework
中间件实现层，主要放置与虚拟机相关的实现代码，是驱动及应用间的桥梁，虚拟机的入口文件为`vm.c`

### include
头文件目录，所有的`.h`文件都放置在此目录。

### init
此目录下放置主入口函数以及版本信息等，主入口函数基本上做一些必要的初始化工作后，就进入shell命令行，等待用户键入命令，或者直接运行某个应用软件。

### kernel
内核目录，提供各种核心组件实现。

#### command
实现种命令行工具，在shell命令行下，可键入相关指令直接执行。

#### core
这个目录是整个系统的核心，提供诸如设备管理、驱动管理、设备树、事件管理、异步消息、性能监测等待。

#### fs
文件系统支持目录，实现虚拟文件系统，可挂载各种文件系统。

#### shell
命令行解析器实现。

#### time
定时器相关实现，用红黑树管理，精度纳秒级。

#### xfs
扩展文件系统，为上层应用提供统一的访问接口。

### lib
这个目录存放自行实现的各种底层库，包括`libc`、`libm`、`libx`等相关库函数。

### romdisk
这是编译进系统的romdisk目录，也就是xboot的根文件系统，以cpio格式存储，并在链接目标文件时一起打包。在系统运行时，会自动挂载到根目录。在板级支持包下，存在一个同名的目录，两者之间是overlay的关系，即板级支持包里的romdisk目录会覆盖默认系统源码目录下的romdisk。如特别文件，请放置在板级支持包里的romdisk目录。
...
## tools
提供各种预编译好的各种平台下的二进制工具，包含QEMU模拟器等。
